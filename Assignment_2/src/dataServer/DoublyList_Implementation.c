...
int IsEmpty(List l){
	if (l->first==NULL){     // If head pointer point to NULL --> return 1 (TRUE). The list is empty.
		return 1;
	}
	else{                   // Else return 0; (FALSE).
		return 0;
	}
}

// I check if a Doubly Linked List is full
int IsFull(List l){
	if(Size(l)==Queue_Size){
		return 1;             // Return 1 (TRUE). The list is full.
	}
	else{
...
void Print(List l){
	ListNodestr *node=l->first;  
	while(node!=NULL){
		printf("{  %s  |  %d  } -->\n", node->file, node->item);        // I cross the list and i print the nodes
		node=node->next;
	}
	printf("\n");
}
...
void Remove(List l, ListNode v){
	...
	if(l->first==v){                  // If the node which i want to delete is the first node
	    if(v->next!=NULL){
	    	nxt=v->next; 
	...
	else if(l->last==v){              // If the node which i want to delete is the last node
		pr=v->prev;                   // 
...
void AddFirst(List l, ListNode newnode){
	if(l->first==NULL){              // If head pointer point to NULL
		l->first=newnode;            // Çead pointer point to newnode
...
void AddLast(List l,ListNode newnode){	
	if(l->last==NULL){               // If tail pointer point to NULL
		l->last=newnode;             // Tail pointer point to newnode
		l->first=newnode;            // Çead pointer point to newnode                                              
		newnode->prev=NULL;          // The previous pointer of the newnode point to NULL
		newnode->next=NULL;          // The next pointer of the newnode point to NULL                                            
...
void AddAfter(List l,ListNode newnode, ListNode v ){
    ListNode u;
    if(l->last==v){                   // If last node is equal with v
...
void AddBefore(List l,ListNode newnode, ListNode v ){
    ListNode u;
	if(l->first==v){                 // If first node is equal with v
...
// Return a node 
ListNode make(Item i, char* file_path){

	ListNodestr *newnode=malloc(sizeof(ListNodestr));  
	
	newnode->file=malloc((strlen(file_path)+1) * sizeof(char));
	strcpy(newnode->file, file_path);                 // I initialize the item of the newnode equal with i
